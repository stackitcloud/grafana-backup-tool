...
from grafana_backup.dashboardApi import get_folder, get_folder_permissions, search_folders
...
def save_folder_setting(folder_name, file_name, folder_settings, folder_permissions, folder_path, pretty_print):
    ...
    # NOTICE: The 'folder_permission' file extension had the 's' removed to work
    # with the magical dict logic in restore.py...
...
def get_individual_folder_setting_and_save(
    ...
    with open('{0}'.format(file_path), 'w+') as f:
        for folder in folders:
            ...
            (status_folder_permissions, content_folder_permissions) = get_folder_permissions(
                folder['uid'], grafana_url, http_get_headers, verify_ssl, client_cert, debug
            )
