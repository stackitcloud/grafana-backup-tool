    ...
    def get_first_dashboard_by_page(page):
        ...
        if status == 200 and len(content):
            if sys.version_info[0] > 2:
                ...
                content[0] = {k: to_python2_and_3_compatible_string(str(v)) for k, v in content[0].iteritems()}
...
def delete_library_element(id_, grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/library-elements/{1}'.format(grafana_url, id_), http_get_headers, verify_ssl, client_cert
    )
...
def delete_team(id_, grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete('{0}/api/teams/{1}'.format(grafana_url, id_), http_get_headers, verify_ssl, client_cert)
...
def delete_team_member(user_id, team_id, grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/teams/{1}/members/{2}'.format(grafana_url, team_id, user_id), http_get_headers, verify_ssl, client_cert
    )
...
def delete_annotation(id_, grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/annotations/{1}'.format(grafana_url, id_), http_get_headers, verify_ssl, client_cert, debug
    )
...
def create_alert_channel(payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/alert-notifications'.format(grafana_url), payload, http_post_headers, verify_ssl, client_cert, debug
    )
...
def delete_alert_channel_by_id(id_, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/alert-notifications/{1}'.format(grafana_url, id_), http_post_headers, verify_ssl, client_cert, debug
    )
...
def delete_folder(uid, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/folders/{1}'.format(grafana_url, uid), http_post_headers, verify_ssl, client_cert, debug
    )


def delete_snapshot(key, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/snapshots/{1}'.format(grafana_url, key), http_post_headers, verify_ssl, client_cert, debug
    )


def delete_dashboard_by_uid(uid, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/dashboards/uid/{1}'.format(grafana_url, uid), http_post_headers, verify_ssl, client_cert, debug
    )


def delete_dashboard_by_slug(slug, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/dashboards/db/{1}'.format(grafana_url, slug), http_post_headers, verify_ssl, client_cert, debug
    )


def create_dashboard(payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/dashboards/db'.format(grafana_url), payload, http_post_headers, verify_ssl, client_cert, debug
    )
...
def search_snapshot(grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    ...
    return send_grafana_get(
        '{0}/api/dashboard/snapshots'.format(grafana_url), http_get_headers, verify_ssl, client_cert, debug
    )
...
def create_snapshot(payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/snapshots'.format(grafana_url), payload, http_post_headers, verify_ssl, client_cert, debug
    )


def create_datasource(payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/datasources'.format(grafana_url), payload, http_post_headers, verify_ssl, client_cert, debug
    )


def delete_datasource_by_uid(uid, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/datasources/uid/{1}'.format(grafana_url, uid), http_post_headers, verify_ssl, client_cert, debug
    )


def delete_datasource_by_id(id_, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_delete(
        '{0}/api/datasources/{1}'.format(grafana_url, id_), http_post_headers, verify_ssl, client_cert, debug
    )
...
def search_folders(grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    ...
    return send_grafana_get(
        '{0}/api/search/?type=dash-folder'.format(grafana_url), http_get_headers, verify_ssl, client_cert, debug
    )


def get_folder(uid, grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    (status_code, content) = send_grafana_get(
        '{0}/api/folders/{1}'.format(grafana_url, uid), http_get_headers, verify_ssl, client_cert, debug
    )
...
def get_folder_permissions(uid, grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    (status_code, content) = send_grafana_get(
        '{0}/api/folders/{1}/permissions'.format(grafana_url, uid), http_get_headers, verify_ssl, client_cert, debug
    )
...
def create_folder(payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/folders'.format(grafana_url), payload, http_post_headers, verify_ssl, client_cert, debug
    )
...
def get_version(dashboard_id, version_number, grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    (status_code, content) = send_grafana_get(
        '{0}/api/dashboards/id/{1}/versions/{2}'.format(grafana_url, dashboard_id, version_number),
        http_get_headers,
        verify_ssl,
        client_cert,
        debug,
...
def search_orgs(grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    return send_grafana_get('{0}/api/orgs'.format(grafana_url), http_get_headers, verify_ssl, client_cert, debug)


def get_org(id, grafana_url, http_get_headers, verify_ssl=False, client_cert=None, debug=True):
    return send_grafana_get(
        '{0}/api/orgs/{1}'.format(grafana_url, id), http_get_headers, verify_ssl, client_cert, debug
    )


def create_org(payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/orgs'.format(grafana_url), payload, http_post_headers, verify_ssl, client_cert, debug
    )


def update_org(id, payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_put(
        '{0}/api/orgs/{1}'.format(grafana_url, id), payload, http_post_headers, verify_ssl, client_cert, debug
    )
...
def get_user(id, grafana_url, http_get_headers, verify_ssl=False, client_cert=None, debug=True):
    return send_grafana_get(
        '{0}/api/users/{1}'.format(grafana_url, id), http_get_headers, verify_ssl, client_cert, debug
    )
...
def get_user_org(id, grafana_url, http_get_headers, verify_ssl=False, client_cert=None, debug=True):
    return send_grafana_get(
        '{0}/api/users/{1}/orgs'.format(grafana_url, id), http_get_headers, verify_ssl, client_cert, debug
    )


def create_user(payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/admin/users'.format(grafana_url), payload, http_post_headers, verify_ssl, client_cert, debug
    )


def add_user_to_org(org_id, payload, grafana_url, http_post_headers, verify_ssl, client_cert, debug):
    return send_grafana_post(
        '{0}/api/orgs/{1}/users'.format(grafana_url, org_id), payload, http_post_headers, verify_ssl, client_cert, debug
    )


def search_contact_points(grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    return send_grafana_get(
        '{0}/api/v1/provisioning/contact-points'.format(grafana_url), http_get_headers, verify_ssl, client_cert, debug
    )
...
def search_notification_policies(grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    return send_grafana_get(
        '{0}/api/v1/provisioning/policies'.format(grafana_url), http_get_headers, verify_ssl, client_cert, debug
    )
...
def send_grafana_delete(url, http_get_headers, verify_ssl=False, client_cert=None, debug=True):
    r = requests.delete(url, headers=http_get_headers, verify=verify_ssl, cert=client_cert)
