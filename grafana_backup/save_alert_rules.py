...
from grafana_backup.dashboardApi import get_grafana_version, search_alert_rules
...
def main(args, settings):
    ...
    if minimum_version <= grafana_version:
        ...
        save_alert_rules(
            folder_path, log_file, grafana_url, http_get_headers, verify_ssl, client_cert, debug, pretty_print
        )
    else:
        print(
...
def get_all_alert_rules_in_grafana(grafana_url, http_get_headers, verify_ssl, client_cert, debug):
    (status, content) = search_alert_rules(grafana_url, http_get_headers, verify_ssl, client_cert, debug)
    if status == 200:
        alert_rules = content
...
def save_alert_rules(
    ...
    for alert_rule in alert_rules:
        ...
        print(alert_rule)
        file_path = save_json(alert_rule['uid'], alert_rule, folder_path, 'alert_rule', pretty_print)
        print(
