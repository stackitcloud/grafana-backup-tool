...
def main(args, settings):
    ...
    get_individual_team_members_and_save(
        teams, folderpath, log_file, pretty_print, grafana_url, http_get_headers, verify_ssl, client_cert, debug
    )
...
def get_individual_team_members_and_save(
    teams, folder_path, log_file, pretty_print, grafana_url, http_get_headers, verify_ssl, client_cert, debug
):
        ...
        with open('{0}'.format(file_path), 'w') as f:
            for team in teams:
                for team_member in get_team_members_in_grafana(
                    team['id'], grafana_url, http_get_headers, verify_ssl, client_cert, debug
                ):
